...
var beat = Object.fromEntries(voice_timeline_default.map((b) => [b.id, b]));
function els(sel) {
...
var order = ["hook1", "hook2", "value1", "bridge", "ai1", "recap1", "recap2", "cta"];
var panelIds = {
  hook1: "#p-hook1",
  hook2: "#p-hook2",
  value1: "#p-value",
...
var OUTRO_HOLD = 3.6;
var END_FADE_DUR = 0.6;
var lastBeat = beat.cta;
var TOTAL_DURATION = lastBeat.start + lastBeat.duration + OUTRO_HOLD + END_FADE_DUR;
order.forEach((id, i) => {
  const b = beat[id];
...
widthGrow("#search-typed", { at: beat.hook1.start + 0.3, dur: 1, to: 430 });
...
var ctaStart = beat.cta.start;
fadeIn(".outro-brand", { at: ctaStart + 0.5, dur: 0.6, y: 16 });
fadeIn(".outro-tag", { at: ctaStart + 1, dur: 0.5, y: 10 });
...
controls.push(
  animate(".cta-btn", { scale: [1, 1, 1.035, 1, 1.035, 1] }, { delay: ctaStart + 2.3, duration: 3, times: [0, 0.15, 0.4, 0.55, 0.8, 1], ease: "ease-in-out" })
);
...
var subtitleEls = Object.fromEntries(voice_timeline_default.map((b) => [b.id, document.getElementById(`sub-${b.id}`)]));
window.__setTime = (t) => {
  for (const c of controls) c.time = t;
  for (const b of voice_timeline_default) {
    const elSub = subtitleEls[b.id];
    if (!elSub) continue;
    const active = t >= b.start - 0.05 && t <= b.start + b.duration + 0.15;
    if (active) {
      elSub.classList.add("is-active");
      const fadeIn2 = Math.min(1, (t - (b.start - 0.05)) / 0.25);
      const fadeOut = Math.min(1, (b.start + b.duration + 0.15 - t) / 0.25);
      elSub.style.opacity = String(Math.max(0, Math.min(fadeIn2, fadeOut)));
    } else {
