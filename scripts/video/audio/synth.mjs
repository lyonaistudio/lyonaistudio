...
// Section energy (0 = silent, 1 = full band) matched to the video's cuts.
function energyAt(t) {
  if (t < 1.5) return 0.0; // hook: near silence, let the whoosh/tension breathe
  if (t < 4.6) return 0.35; // logo reveal: pad only, building
  if (t < 9.6) return 0.55; // value prop: pad + soft arp
  if (t < 21.6) return 1.0; // product + services: full groove
  if (t < 25.1) return 0.4; // chat demo: pull back so the "ding" reads clearly
  if (t < 29.1) return 0.85; // steps: building back up
  return 1.0; // outro / CTA: peak
}
