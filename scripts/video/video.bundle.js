...
// video.mjs
var controls = [];
var OVERSHOOT = "cubic-bezier(0.34, 1.56, 0.64, 1)";
function els(sel) {
  return Array.from(document.querySelectorAll(sel));
}
function fadeIn(target, { at, dur = 0.5, y = 0, x = 0, scale: scale2, ease: ease2 = "ease-out" }) {
  const opacity = [0, 1];
  const transform = {};
  if (y) transform.y = [y, 0];
  if (x) transform.x = [x, 0];
  if (scale2) transform.scale = [scale2, 1];
  controls.push(animate(target, { opacity, ...transform }, { delay: at, duration: dur, ease: ease2 }));
}
function slideOut(target, { at, dur = 0.5, y = 0, x = 0, ease: ease2 = "ease-in" }) {
  const transform = {};
  if (y) transform.y = [0, y];
  if (x) transform.x = [0, x];
  controls.push(animate(target, transform, { delay: at, duration: dur, ease: ease2 }));
}
function fadeInOut(target, { inAt, inDur = 0.5, outAt, outDur = 0.5, y = 0, x = 0, ease: ease2 = "ease-in-out" }) {
  ...
  controls.push(animate(target, { opacity, ...transform }, { delay: inAt, duration: total, times, ease: ease2 }));
}
function panY(target, { at, dur, to, ease: ease2 = "linear" }) {
  controls.push(animate(target, { y: [0, to] }, { delay: at, duration: dur, ease: ease2 }));
}
function grow(target, { at, dur, from = 0, ease: ease2 = OVERSHOOT }) {
  controls.push(animate(target, { scale: [from, 1], opacity: [0, 1] }, { delay: at, duration: dur, ease: ease2 }));
}
function moveVanish(target, { at, dur, toX, toY, ease: ease2 = "ease-in" }) {
  controls.push(
    animate(target, { x: [0, toX], y: [0, toY], scale: [1, 0.3], opacity: [1, 0] }, { delay: at, duration: dur, ease: ease2 })
  );
}
...
moveVanish(".hook-pill:nth-of-type(1)", { at: 1, dur: 0.35, toX: 450, toY: 590 });
moveVanish(".hook-pill:nth-of-type(2)", { at: 1, dur: 0.35, toX: -250, toY: 90 });
moveVanish(".hook-pill:nth-of-type(3)", { at: 1, dur: 0.35, toX: 400, toY: -420 });
...
fadeIn("#p-value .eyebrow", { at: 5, dur: 0.4, y: 12 });
...
fadeIn("#p-product .eyebrow", { at: 10, dur: 0.4, y: 12 });
...
fadeIn("#p-services .eyebrow", { at: 16, dur: 0.4, y: 12 });
...
fadeIn("#p-chat .eyebrow", { at: 22, dur: 0.4, y: 12 });
...
fadeIn(".outro-cta", { at: 31, dur: 0.4, scale: 0.85 });
fadeIn(".outro-qr", { at: 31.4, dur: 0.4, y: 10 });
controls.push(
  animate(
    ".cta-btn",
    { scale: [1, 1, 1.035, 1, 1.035, 1] },
    { delay: 31.4, duration: 3, times: [0, 0.15, 0.4, 0.55, 0.8, 1], ease: "ease-in-out" }
  )
);
controls.push(
  animate(
